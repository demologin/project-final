...
public interface ActivityRepository extends BaseRepository<Activity> {
    ...
    @Query("SELECT a FROM Activity a JOIN FETCH a.author WHERE a.taskId =:taskId AND a.statusCode =:status ORDER BY a.updated DESC")
    List<Activity> findAllByTaskIdOrderAndStatus(long taskId, String status);
